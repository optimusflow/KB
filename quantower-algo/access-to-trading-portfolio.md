...
This is only the main properties from [Core](http://api.optimusflow.qtower.app/docs/TradingPlatform.BusinessLayer.Core.html). You can check our [API documentation](http://api.optimusflow.qtower.app/index.html) and found many other interesting's ones: Symbol types, Assets, Reports. Actually, you can get all data that are used by OptimusFLOW trading platform, as we use the same API during development our plugins.
