...
The backup process is easy and doesn’t require many explanations. Just select the path where your backup file will be stored \(OptimusFLOW backups folder by default; recommended\) and press the **\[ CREATE BACKUP \]** button.
...
{% hint style="info" %}
You may find some backup files in OptimusFLOW Backup folder, containing the “**A**” letter at the end — these are the Automatic backups, periodically made by OptimusFLOW \(daily, overridden each 5 min\).
{% endhint %}

## Restore settings

In case you want to restore some previous settings or wish to apply some custom settings \(got from another OptimusFLOW user, that backed up its settings previously\), follow the **\[ RECOVER SETTINGS \]** button.
...
The “_**Backups folder**_” is the default OptimusFLOW folder where it saves all automatically created backup files and proposes it to store yours, manually created. The Backup & restore manager checks this folder and displays you all the found files to import settings. You may also select some different location of your backup file, using the “_**From custom file**_” option.
...
{% hint style="success" %}
In case of any problems with the backup file, B&R manager will keep your current OptimusFLOW settings and inform you about it.
{% endhint %}
